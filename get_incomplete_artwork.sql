DELIMITER $$
DROP PROCEDURE IF EXISTS get_incomplete_artwork $$

CREATE PROCEDURE get_incomplete_artwork(IN section TINYINT(1))
script:BEGIN
...
DECLARE having_part NVARCHAR(1000);
...
# CASING TYPE #######################################################
IF having_part IS NULL THEN SET having_part = " HAVING "; END IF;
...
get_results: LOOP
	...
	IF having_part = " HAVING "
	THEN SET having_part := CONCAT(having_part, type_name_f, ' > 0');
	ELSE SET having_part := CONCAT(having_part,' OR ', type_name_f, ' > 0');
	END IF;
